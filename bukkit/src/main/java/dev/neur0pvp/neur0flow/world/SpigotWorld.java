    ...
    static {
        if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThan(ServerVersion.V_1_13)) {
            try {
                ...
                Class<?> craftWorldClass = Class.forName(bukkitPackage + ".CraftWorld");
                vec3DClass = Class.forName(nmsPackage + ".Vec3D");
                Class<?> movingObjectPositionClass = Class.forName(nmsPackage + ".MovingObjectPosition");
    ...
    public RayTraceResult rayTraceBlocks(Vector3d start, Vector3d direction, double maxDistance, FluidHandling fluidHandling, boolean ignorePassableBlocks) {
        if (PacketEvents.getAPI().getServerManager().getVersion().isNewerThan(ServerVersion.V_1_12_2)) {
            ...
            return new RayTraceResult(
                    ...
                    getBlockFaceFrom(Objects.requireNonNull(result.getHitBlockFace())),
                    new Vector3i(Objects.requireNonNull(result.getHitBlock()).getX(), result.getHitBlock().getY(), result.getHitBlock().getZ()),
            ...
            try {
                ...
                return new RayTraceResult(
                        ...
                        WrappedBlockState.getByString(hitBlock.getClass().getField("type").get(hitBlock).toString())
                );
            } catch (Exception e) {
                e.printStackTrace();
